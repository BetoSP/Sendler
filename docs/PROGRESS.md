...
| 3 | PWA Asistentes (login, guardias, GPS, reporte + IA) | 🟡 En progreso — primer corte construido y verificado en navegador real (login, Mis Guardias, check-in/out GPS, Reporte Diario con IA Nivel 1, Mi Perfil); sin desplegar a Vercel todavía; push notifications, alertas tempranas de ausencia y WebAuthn quedaron fuera de este primer corte a pedido del Desarrollador |
...
Fase 2 del plan "Terminar la Etapa 2 (Panel): carga de datos, permisos configurables e
importación masiva" (`docs/PENDIENTES.md` referencia el plan aprobado) — motor de permisos
configurable por Prestadora, verificado de punta a punta en Prestadora Demo con una sesión
real de coordinador contra Supabase real, y desplegado a producción
(`https://aurevia-panel.vercel.app`). Detalle completo en la fila 2026-07-19 (Fase 2) de
"Archivos creados/modificados por sesión" más abajo.
Próximo paso: Fase 3 (importación masiva de datos con IA), no arrancada todavía — el plan
exige Fase 2 verificada antes de tocarla.
